...
async function trackText(track) {
  ...
  throw new CaptionError('fetch', `YouTube listed a French track but gave no text for it (${tries.join('; ')}).`, { step: 'caption text', tries });
}
...
async function fetchVideo(videoId) {
  ...
  const lines = await trackText(chosen);
  return {
    youtube_id: videoId,
...
module.exports = { parseLink, parseTime, parseCaptions, chooseTrack, fetchVideo, oembedTitle, CaptionError, decodeEntities };
