...
//                 "Which pattern?" (four names: one of the line's own and
//                 three it does not have).
//
...
// Four pattern ids: one of the line's own, and three it does not have. With
// more of its own among them the question would answer itself.
function patternChoices(patterns, rand) {
  const own = [...new Set(patterns)];
  const one = own[Math.floor(rand() * own.length)];
  const rest = shuffle(IDS.filter((id) => !own.includes(id)), rand).slice(0, 3);
  return shuffle([one, ...rest], rand);
}
