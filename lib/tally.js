...
//                 in Ear first without a "What was that?" on them, or in ten
//                 different lines he got right in a drill ("What was said?"),
//                 and no "got past me" for it among his last 200 events.
//                 A drill answer weighs below Ear first: ten, not five. A
//                 wrong drill answer is a "got past me" like any other.
...
function states(events) {
  ...
  for (const p of PATTERNS) {
    ...
    const drillLines = new Set(mine.filter((e) => e.kind === 'drill_clean').map((e) => e.line_id));
    const counts = {
      ...
      drill_clean: drillLines.size,
    };
    let state;
    if (mine.length === 0) state = 'not-met';
    else if (mineRecent.some((e) => e.kind === 'got_past_me' || e.kind === 'keep')) state = 'shaky';
    else if ((cleanLines.size >= SOLID_LINES || drillLines.size >= SOLID_DRILL_LINES)
      && !mineRecent.some((e) => e.kind === 'got_past_me')) state = 'solid';
    else state = 'seen';
...
module.exports = { states, RECENT, SOLID_LINES, SOLID_DRILL_LINES };
